...
Codemagic supports both "automatic" and "manual" processes to code sign your app. In this tutorial, we will only cover automatic code signing, as it is by far the easier process, especially for newcomers to iOS code signing.
...
Once you have finished testing and are ready to publish your app on the App Store, the final step is to submit it for review. To do this, simply run your build on Codemagic again, this time enabling the option under the Codemagic App Store Connect section to submit the app for review:
